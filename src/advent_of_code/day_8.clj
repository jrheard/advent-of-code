...
(s/def ::item (s/alt :hex-escaped-char ::hex-escaped-char
                     ...
                     :escaped-quote (s/cat :slash ::backslash :quote ::quote)
                     :bare-quote ::quote
                     :plain-char char?))
...
(defn encode-list-line [list-line]
  (mapcat (fn [item]
            (let [[item-type item-contents] (first (vals item))]
              (case item-type
                :plain-char [item-contents]
                :backslash [\\ \\ \\ \\]
                :escaped-quote [\\ \\ \\ \"]
                :bare-quote [\\ \"]
                :hex-escaped-char (conj (vals item-contents) \\))))
          list-line))

(defn encoded-length [list-line]
  (-> (encode-list-line list-line)
      count
      (+ 2)))

(def part-2
  (let [original-code-length (apply + (map code-length list-lines))
        encoded-code-length (apply + (map encoded-length list-lines))]
    (- encoded-code-length original-code-length)))
